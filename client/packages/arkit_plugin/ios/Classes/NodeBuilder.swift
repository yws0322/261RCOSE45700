...
func updateNode(_ node: SCNNode, fromDict dict: [String: Any], forDevice device: MTLDevice?) {
    ...
    if let categoryBitMask = dict["categoryBitMask"] as? Int {
        // Apply to the whole hierarchy: glTF models are loaded as child nodes
        // of a container, and SceneKit does not propagate category masks.
        node.enumerateHierarchy { child, _ in
            child.categoryBitMask = categoryBitMask
        }
    }
