...
class ARKitController {
  ...
  /// Samples the luminance of the *raw camera image* (no virtual content) at
  /// the given world-space points, by reading the camera frame's Y plane.
  ///
  /// [points] is a flat list of world coordinates: x0,y0,z0,x1,y1,z1,...
  ///
  /// Returns a map with:
  /// - 'samples': List of luminance values in 0..1, one per point; -1 when a
  ///   point is behind the camera or projects outside the frame;
  /// - 'frameAverage': average luminance of the whole frame in 0..1.
  ///
  /// Returns null when no camera frame is available yet.
  Future<Map<String, dynamic>?> sampleCameraLuminance(
      List<double> points) async {
    final result = await _channel.invokeMapMethod<String, dynamic>(
        'sampleCameraLuminance', {'points': points});
    return result;
  }
